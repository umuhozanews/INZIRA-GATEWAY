...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Offline / Local storage reactive calculation
      const sales = StorageEngine.getSales().filter((s) => !s.is_voided);
      const expenses = StorageEngine.getExpenses();
      const stock = StorageEngine.getStock();

      const todayStr = new Date().toISOString().split("T")[0];
      const todaySales = sales.filter((s) => s.created_at?.startsWith(todayStr) || s.created_at?.includes(todayStr));
      const todayExp = expenses.filter((e) => e.expense_date === todayStr || e.created_at?.startsWith(todayStr));

      const totalRevToday = todaySales.reduce((sum, s) => sum + (Number(s.amount_paid || s.total_amount) || 0), 0);
      const totalExpToday = todayExp.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);

      const totalRevAllTime = sales.reduce((sum, s) => sum + (Number(s.amount_paid || s.total_amount) || 0), 0);
      const totalExpAllTime = expenses.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);

      const displayRev = totalRevToday > 0 ? totalRevToday : totalRevAllTime;
      const displayExp = totalExpToday > 0 ? totalExpToday : totalExpAllTime;

      const lowStockItems = stock.filter((i) => Number(i.quantity) <= Number(i.low_stock_threshold || 5));

      setStats({
        todayRevenue: displayRev,
        netCashToday: displayRev - displayExp,
        salesChangePct: 14.2,
        healthScore: { score: 85, band: "A" },
        alerts: lowStockItems.map((item) => ({ type: "low_stock", message: `Low Stock: ${item.name} (${item.quantity} in stock)` }))
      });
    ...
    <div className="pb-6">
      ...
      {lowAlert && (
        ...
        >
          ...
          <ChevronRight size={15} className="text-danger shrink-0" />
        </button>
      )}
        ...
        <div className="mt-2 grid grid-cols-4 gap-2.5">
          {quickActions.map(({ label, icon: Icon, to }) => (
            <button key={label} onClick={() => navigate(to)} className="flex flex-col items-center gap-1.5">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary-xlt shadow-xs active:scale-95 transition-all">
      ...
      <div className="mt-4 px-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => navigate("/reports/sales")} className="text-[11px] font-bold text-primary">
            View All Sales
          </button>
        </div>
        <div className="mt-2 flex flex-col gap-2">
          {recent.length === 0 ? (
            <div className="rounded-xl border border-line bg-card p-4 text-center text-[12px] text-muted">
              No sales recorded yet. Tap "Record Sale" to start.
            </div>
          ) : (
            recent.map((s) => (
              <div key={s.id} className="flex items-center justify-between rounded-xl border border-line bg-card p-3 shadow-xs">
                <div>
                  <div className="text-[13px] font-bold text-ink">{s.customer_name}</div>
                  <div className="text-[10.5px] text-muted">{s.invoice_number} • {s.payment_method?.toUpperCase()}</div>
                </div>
                <div className="text-right">
                  <div className="text-[13px] font-bold text-emerald-600 tabnum">+{rwf(s.total_amount)} RWF</div>
                  <div className="text-[10px] text-muted">{clockTime(s.created_at)}</div>
                </div>
              </div>
            ))
          )}
